...
const SetListView = () => {
    ...
    return (
        <div className="bg-muted">
            ...
            {setlist && <h2 className="print:hidden text-center text-sm md:text-base lg:text-lg font-bold sticky top-0 left-0 z-10 w-full bg-primary text-primary-foreground py-4 shadow-md flex items-center gap-2 justify-center"><span>{setlist.name}</span><button onClick={handlePrint} className="flex items-center justify-center gap-1 bg-muted hover:bg-accent p-2 rounded font-normal"><PrinterIcon size={18} /> Print</button></h2>}
            ...
            {outputs.map((output) =>
                <div
                    ...
                    className="columns-1 lg:columns-2 sheet overflow-x-auto text-[12px] xl:text-base mt-4 bg-card shadow-lg rounded-lg p-6 max-w-3xl xl:max-w-5xl w-full border border-border"
                    style={{columnGap: '20px'}}
                />
            )}
            </div>
            <footer className="print:hidden text-center text-sm text-primary-foreground w-full bg-primary">
