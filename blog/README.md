...
The blog now emits canonical URLs, Open Graph and Twitter metadata, JSON-LD, a sitemap, an RSS feed, and a crawl policy route.

- `GET /robots.txt` points crawlers to the generated sitemap.
- `GET /sitemap-index.xml` is generated by Astro's sitemap integration.
- `GET /rss.xml` exposes the stories feed.

Shared page metadata lives in `src/layouts/BaseLayout.astro`, and story pages can pass article publish/modified timestamps plus social image overrides.
